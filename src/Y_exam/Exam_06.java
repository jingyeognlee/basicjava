...
//		}	
		...
		*/
		...
		class Student {
			...
			float getAverage(){
				float avg = (int)((float)sum /3 *100 +0.5f)/100f;
				return avg;
			}
		...
		*/
		...
		/*[6-10] 다음 중 생성자에 대한 설명으로 옳지 않은 것은? (모두 고르시오) b,
		a. 모든 생성자의 이름은 클래스의 이름과 동일해야한다.
		b. 생성자는 객체를 생성하기 위한 것이다. -- 인스턴스를 
		...
		long add(int a, int b) { return a+b;} 안됨
		a. long add(int x, int y) { return x+y;} 됨
		b. long add(long a, long b) { return a+b;} 
		c. int add(byte a, byte b) { return a+b;}됨
		d. int add(long a, int b) { return (int)(a+b);}*/됨
		
		
		/*[6-14] 다음 중 초기화에 대한 설명으로 옳지 않은 것은? (모두 고르시오)
		a.멤버변수는 자동 초기화되므로 초기화하지 않고도 값을 참조할 수 있다. -기본값 초기화
		b.지역변수는 사용하기 전에 반드시 초기화해야 한다. o
		c.초기화 블럭보다 생성자가 먼저 수행된다. 블럭부터
		d.명시적 초기화를 제일 우선적으로 고려해야 한다. o
		e.클래스변수보다 인스턴스변수가 먼저 초기화된다.*/ 클래스부터
...
e. 힙(heap)영역에 생성되며 가비지 컬렉터에 의해 소멸된다. *///callstack 메서드안에 선언
		...
		이유를 설명하시오. 클래스 친구가 인스턴스 친구를 호출할 수 없는 이유
		class MemberCall {
		int iv = 10;
		static int cv = 20;
		int iv2 = cv;
